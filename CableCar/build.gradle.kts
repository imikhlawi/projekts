...
repositories {
    mavenCentral()
    val sopraToken = System.getenv("SOPRA_GITLAB_TOKEN")
    if (sopraToken != null && sopraToken.isNotBlank()) {
        maven {
...
tasks.jacocoTestReport {
    ...
    classDirectories.setFrom(files(classDirectories.files.map {
        fileTree(it) {
            exclude(listOf("view/**", "entity/serializer.", "Main*.*", "ai/**"))
        }
    }))
}
